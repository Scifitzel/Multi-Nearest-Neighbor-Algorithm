...
whichWay way = UP;
 ...
 for (int i = 0; i < rawPoints.size(); i++){
    ...
    bool reachedSecondPointTemp = false;
...
while (pointToStartAt != points.size()) {
    ...
    float total = 0;
    int index = 0;
    reachedSecondPointTemp = reachedSecondPoint;
    while (startingpoint != points.size()){
        std::vector<int> pointsAlreadyUsed{};
        pointsAlreadyUsed = pointsAlreadyTraveledSorted;
        
        while (startingpoint < points.size() && (std::binary_search(pointsAlreadyUsed.begin(),pointsAlreadyUsed.end(), startingpoint)))  {     
            startingpoint++;
        }
        ...
        while (pointsAlreadyUsed.size() != points.size()){
            ...
            for (int i = 0; i <  points[currentPoint].size(); i+=2){
                if ( !std::binary_search(pointsAlreadyUsed.begin(),pointsAlreadyUsed.end(), points[currentPoint][i+1]) &&  !(( (way == UP &&  rawPoints[firstPoint].y-rawPoints[startingpoint].y < 0) ||   (way == DOWN &&  rawPoints[firstPoint].y-rawPoints[startingpoint].y > 0) || ((way == RIGHT &&  rawPoints[firstPoint].x-rawPoints[startingpoint].x > 0) || (way == LEFT && rawPoints[firstPoint].x-rawPoints[secondPoint].x < 0))) && (!reachedSecondPointTemp) )   ){ // there is no need to check if it is the smallest  because it is already sorted
                    index = i+1;
                    break;
                    }
                }
